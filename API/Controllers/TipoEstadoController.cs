...
public class TipoEstadoController : BaseApiController
{
    ...
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<ActionResult<Pager<TipoEstadoDto>>> GetPaginacion([FromQuery] Params TipoEstadoParams)
    {
        var entidad = await unitofwork.TipoEstados.GetAllAsync(TipoEstadoParams.PageIndex, TipoEstadoParams.PageSize, TipoEstadoParams.Search);
        var listEntidad = mapper.Map<List<TipoEstadoDto>>(entidad.registros);
        return new Pager<TipoEstadoDto>(listEntidad, entidad.totalRegistros, TipoEstadoParams.PageIndex, TipoEstadoParams.PageSize, TipoEstadoParams.Search);
    }

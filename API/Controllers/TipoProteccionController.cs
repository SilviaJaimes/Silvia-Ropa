...
public class TipoProteccionController : BaseApiController
{
    ...
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<ActionResult<Pager<TipoProteccionDto>>> GetPaginacion([FromQuery] Params TipoProteccionParams)
    {
        var entidad = await unitofwork.TipoProtecciones.GetAllAsync(TipoProteccionParams.PageIndex, TipoProteccionParams.PageSize, TipoProteccionParams.Search);
        var listEntidad = mapper.Map<List<TipoProteccionDto>>(entidad.registros);
        return new Pager<TipoProteccionDto>(listEntidad, entidad.totalRegistros, TipoProteccionParams.PageIndex, TipoProteccionParams.PageSize, TipoProteccionParams.Search);
    }

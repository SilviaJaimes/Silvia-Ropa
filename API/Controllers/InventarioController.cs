...
public class InventarioController : BaseApiController
{
    ...
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<ActionResult<Pager<InventarioDto>>> GetPaginacion([FromQuery] Params InventarioParams)
    {
        var entidad = await unitofwork.Inventarios.GetAllAsync(InventarioParams.PageIndex, InventarioParams.PageSize, InventarioParams.Search);
        var listEntidad = mapper.Map<List<InventarioDto>>(entidad.registros);
        return new Pager<InventarioDto>(listEntidad, entidad.totalRegistros, InventarioParams.PageIndex, InventarioParams.PageSize, InventarioParams.Search);
    }
    ...
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<ActionResult<object>> ProductosYTallas()
    {
        var entidad = await unitofwork.Inventarios.ProductosYTallas();

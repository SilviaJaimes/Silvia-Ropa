...
public class MunicipioController : BaseApiController
{
    ...
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<ActionResult<Pager<MunicipioDto>>> GetPaginacion([FromQuery] Params MunicipioParams)
    {
        var entidad = await unitofwork.Municipios.GetAllAsync(MunicipioParams.PageIndex, MunicipioParams.PageSize, MunicipioParams.Search);
        var listEntidad = mapper.Map<List<MunicipioDto>>(entidad.registros);
        return new Pager<MunicipioDto>(listEntidad, entidad.totalRegistros, MunicipioParams.PageIndex, MunicipioParams.PageSize, MunicipioParams.Search);
    }

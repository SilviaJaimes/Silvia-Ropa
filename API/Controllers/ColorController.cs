...
public class ColorController : BaseApiController
{
    ...
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<ActionResult<Pager<ColorDto>>> GetPaginacion([FromQuery] Params ColorParams)
    {
        var entidad = await unitofwork.Colores.GetAllAsync(ColorParams.PageIndex, ColorParams.PageSize, ColorParams.Search);
        var listEntidad = mapper.Map<List<ColorDto>>(entidad.registros);
        return new Pager<ColorDto>(listEntidad, entidad.totalRegistros, ColorParams.PageIndex, ColorParams.PageSize, ColorParams.Search);
    }

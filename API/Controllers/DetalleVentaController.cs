...
public class DetalleVentaController : BaseApiController
{
    ...
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<ActionResult<Pager<DetalleVentaDto>>> GetPaginacion([FromQuery] Params DetalleVentaParams)
    {
        var entidad = await unitofwork.DetalleVentas.GetAllAsync(DetalleVentaParams.PageIndex, DetalleVentaParams.PageSize, DetalleVentaParams.Search);
        var listEntidad = mapper.Map<List<DetalleVentaDto>>(entidad.registros);
        return new Pager<DetalleVentaDto>(listEntidad, entidad.totalRegistros, DetalleVentaParams.PageIndex, DetalleVentaParams.PageSize, DetalleVentaParams.Search);
    }

...
public class GeneroController : BaseApiController
{
    ...
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<ActionResult<Pager<GeneroDto>>> GetPaginacion([FromQuery] Params GeneroParams)
    {
        var entidad = await unitofwork.Generos.GetAllAsync(GeneroParams.PageIndex, GeneroParams.PageSize, GeneroParams.Search);
        var listEntidad = mapper.Map<List<GeneroDto>>(entidad.registros);
        return new Pager<GeneroDto>(listEntidad, entidad.totalRegistros, GeneroParams.PageIndex, GeneroParams.PageSize, GeneroParams.Search);
    }

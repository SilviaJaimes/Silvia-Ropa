...
public class InsumoPrendaController : BaseApiController
{
    ...
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<ActionResult<Pager<InsumoPrendaDto>>> GetPaginacion([FromQuery] Params InsumoPrendaParams)
    {
        var entidad = await unitofwork.InsumoPrendas.GetAllAsync(InsumoPrendaParams.PageIndex, InsumoPrendaParams.PageSize, InsumoPrendaParams.Search);
        var listEntidad = mapper.Map<List<InsumoPrendaDto>>(entidad.registros);
        return new Pager<InsumoPrendaDto>(listEntidad, entidad.totalRegistros, InsumoPrendaParams.PageIndex, InsumoPrendaParams.PageSize, InsumoPrendaParams.Search);
    }

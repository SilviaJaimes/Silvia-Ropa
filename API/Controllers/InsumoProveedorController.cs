...
public class InsumoProveedorController : BaseApiController
{
    ...
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<ActionResult<Pager<InsumoProveedorDto>>> GetPaginacion([FromQuery] Params InsumoProveedorParams)
    {
        var entidad = await unitofwork.InsumoProveedores.GetAllAsync(InsumoProveedorParams.PageIndex, InsumoProveedorParams.PageSize, InsumoProveedorParams.Search);
        var listEntidad = mapper.Map<List<InsumoProveedorDto>>(entidad.registros);
        return new Pager<InsumoProveedorDto>(listEntidad, entidad.totalRegistros, InsumoProveedorParams.PageIndex, InsumoProveedorParams.PageSize, InsumoProveedorParams.Search);
    }

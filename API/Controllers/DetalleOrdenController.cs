...
public class DetalleOrdenController : BaseApiController
{
    ...
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<ActionResult<Pager<DetalleOrdenDto>>> GetPaginacion([FromQuery] Params DetalleOrdenParams)
    {
        var entidad = await unitofwork.DetalleOrdenes.GetAllAsync(DetalleOrdenParams.PageIndex, DetalleOrdenParams.PageSize, DetalleOrdenParams.Search);
        var listEntidad = mapper.Map<List<DetalleOrdenDto>>(entidad.registros);
        return new Pager<DetalleOrdenDto>(listEntidad, entidad.totalRegistros, DetalleOrdenParams.PageIndex, DetalleOrdenParams.PageSize, DetalleOrdenParams.Search);
    }

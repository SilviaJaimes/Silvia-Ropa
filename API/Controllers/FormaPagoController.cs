...
public class FormaPagoController : BaseApiController
{
    ...
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<ActionResult<Pager<FormaPagoDto>>> GetPaginacion([FromQuery] Params FormaPagoParams)
    {
        var entidad = await unitofwork.FormaPagos.GetAllAsync(FormaPagoParams.PageIndex, FormaPagoParams.PageSize, FormaPagoParams.Search);
        var listEntidad = mapper.Map<List<FormaPagoDto>>(entidad.registros);
        return new Pager<FormaPagoDto>(listEntidad, entidad.totalRegistros, FormaPagoParams.PageIndex, FormaPagoParams.PageSize, FormaPagoParams.Search);
    }

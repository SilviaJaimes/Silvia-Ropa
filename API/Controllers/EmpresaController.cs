...
public class EmpresaController : BaseApiController
{
    ...
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<ActionResult<Pager<EmpresaDto>>> GetPaginacion([FromQuery] Params EmpresaParams)
    {
        var entidad = await unitofwork.Empresas.GetAllAsync(EmpresaParams.PageIndex, EmpresaParams.PageSize, EmpresaParams.Search);
        var listEntidad = mapper.Map<List<EmpresaDto>>(entidad.registros);
        return new Pager<EmpresaDto>(listEntidad, entidad.totalRegistros, EmpresaParams.PageIndex, EmpresaParams.PageSize, EmpresaParams.Search);
    }

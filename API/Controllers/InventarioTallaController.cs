...
public class InventarioTallaController : BaseApiController
{
    ...
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<ActionResult<Pager<InventarioTallaDto>>> GetPaginacion([FromQuery] Params InventarioTallaParams)
    {
        var entidad = await unitofwork.InventarioTallas.GetAllAsync(InventarioTallaParams.PageIndex, InventarioTallaParams.PageSize, InventarioTallaParams.Search);
        var listEntidad = mapper.Map<List<InventarioTallaDto>>(entidad.registros);
        return new Pager<InventarioTallaDto>(listEntidad, entidad.totalRegistros, InventarioTallaParams.PageIndex, InventarioTallaParams.PageSize, InventarioTallaParams.Search);
    }

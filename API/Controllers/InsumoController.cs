...
public class InsumoController : BaseApiController
{
    ...
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<ActionResult<Pager<InsumoDto>>> GetPaginacion([FromQuery] Params InsumoParams)
    {
        var entidad = await unitofwork.Insumos.GetAllAsync(InsumoParams.PageIndex, InsumoParams.PageSize, InsumoParams.Search);
        var listEntidad = mapper.Map<List<InsumoDto>>(entidad.registros);
        return new Pager<InsumoDto>(listEntidad, entidad.totalRegistros, InsumoParams.PageIndex, InsumoParams.PageSize, InsumoParams.Search);
    }

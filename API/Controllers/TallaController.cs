...
public class TallaController : BaseApiController
{
    ...
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<ActionResult<Pager<TallaDto>>> GetPaginacion([FromQuery] Params TallaParams)
    {
        var entidad = await unitofwork.Tallas.GetAllAsync(TallaParams.PageIndex, TallaParams.PageSize, TallaParams.Search);
        var listEntidad = mapper.Map<List<TallaDto>>(entidad.registros);
        return new Pager<TallaDto>(listEntidad, entidad.totalRegistros, TallaParams.PageIndex, TallaParams.PageSize, TallaParams.Search);
    }

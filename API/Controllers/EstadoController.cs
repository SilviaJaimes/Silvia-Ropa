...
public class EstadoController : BaseApiController
{
    ...
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<ActionResult<Pager<EstadoDto>>> GetPaginacion([FromQuery] Params EstadoParams)
    {
        var entidad = await unitofwork.Estados.GetAllAsync(EstadoParams.PageIndex, EstadoParams.PageSize, EstadoParams.Search);
        var listEntidad = mapper.Map<List<EstadoDto>>(entidad.registros);
        return new Pager<EstadoDto>(listEntidad, entidad.totalRegistros, EstadoParams.PageIndex, EstadoParams.PageSize, EstadoParams.Search);
    }
